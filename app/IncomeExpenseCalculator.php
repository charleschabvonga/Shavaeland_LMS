 ...
 * @property decimal $other_costs
 * @property decimal $total_costs
 * @property decimal $trip_income
 * @property decimal $balance
*/
class IncomeExpenseCalculator extends Model
{
    use SoftDeletes;

    protected $fillable = ['route_id', 'distance', 'load_status', 'truck_attachment_status_id', 'machinery_attachment_type_id', 'size_id', 'vehicles_id', 'purchase_price', 'fuel_usage', 'salvage_value', 'avg_investment', 'depreciation', 'insurance', 'license', 'fuel_price', 'fuel_usage', 'fuel', 'fuel_consumption', 'oil_price', 'oil_usage', 'oil', 'oil_consumption', 'tyre_price', 'number_of_tyres', 'tyre', 'repair_maintenance', 'contigency_factor', 'total_costs', 'other_costs', 'trip_income', 'balance'];
    protected $hidden = [];
    ...
    public static function boot()
    {
        ...
        IncomeExpenseCalculator::observe(new \App\Observers\UserActionsObserver);
    }
     ...
     * @param $input
     */
    public function setVehiclesIdAttribute($input)
    {
        $this->attributes['vehicles_id'] = $input ? $input : null;
    }
    ...
    {
        if ($this->attributes['machinery_attachment_type_id'] == 1 || $this->attributes['machinery_attachment_type_id'] == 2) {
             $lifePeriodKm = 300000;
        } 
        else {
             $lifePeriodKm = 700000;
    ...
    {
        if ($this->attributes['machinery_attachment_type_id'] == 1 && ($this->attributes['size_id'] == 1 || $this->attributes['size_id'] == 2)) {
             $kmPerAnnum = 35000;
        }
        else if ($this->attributes['machinery_attachment_type_id'] == 1 && $this->attributes['size_id'] == 3) {
             $kmPerAnnum = 40000;
        }
        else if ($this->attributes['machinery_attachment_type_id'] == 1 && ($this->attributes['size_id'] == 4 || $this->attributes['size_id'] == 5)) {
             $kmPerAnnum = 60000;
        } 
        else {
             $kmPerAnnum = 70000;
    ...
    public function setLicenseAttribute($input)
    {
        ...
        else if ($this->attributes['size_id'] == 10) {
    ...
    public function setFuelUsageAttribute($input)
    {
        ...
        else if($this->attributes['size_id'] == 9) {
            $this->attributes['fuel_usage'] = 50;
        }
        else if($this->attributes['size_id'] == 10 || $this->attributes['size_id'] == 11 || $this->attributes['size_id'] == 14) {
            $this->attributes['fuel_usage'] = 45;
        }
        else if($this->attributes['size_id'] == 12 || $this->attributes['size_id'] == 15) {
            $this->attributes['fuel_usage'] = 47;
        }
        else{
     ...
     * @param $input
     */
    public function setTripIncomeAttribute($input)
    {
        $this->attributes['trip_income'] = $input ? $input : null;
    }
     ...
     * @param $input
     */
    public function setOtherCostsAttribute($input)
    {
        $this->attributes['other_costs'] = $input ? $input : null;
    }
    ...
    {
        $this->attributes['total_costs'] = $this->attributes['depreciation']+$this->attributes['insurance']+$this->attributes['license']+$this->attributes['repair_maintenance']+$this->attributes['oil']+$this->attributes['tyre']+$this->attributes['contigency_factor']+$this->attributes['fuel'];
    }
    ...
    {
        $this->attributes['balance'] = $this->attributes['trip_income']-($this->attributes['other_costs']+$this->attributes['total_costs']);
    }
