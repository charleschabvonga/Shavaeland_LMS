...
class DeliveryInstruction extends Model
{
    ...
    protected $fillable = ['freight_contract_type', 'delivery_instruction_number', 'order_number', 'delivery_company_name', 'delivery_date_time', 'prepared_by', 'status', 'delivery_address_address', 'delivery_address_latitude', 'delivery_address_longitude', 'road_freight_number_id', 'driver_id', 'vehicle_id', 'vendor_id', 'vendor_driver_id', 'client_id', 'contact_person_id', 'project_manager_id'];
    protected $hidden = [];
    ...
    public static function boot()
    {
        ...
        DeliveryInstruction::observe(new \App\Observers\UserActionsObserver);
    }
     ...
     * @param $input
     */
    public function setDeliveryDateTimeAttribute($input)
    {
        if ($input != null && $input != '') {
            $this->attributes['delivery_date_time'] = Carbon::createFromFormat(config('app.date_format') . ' H:i:s', $input)->format('Y-m-d H:i:s');
        } else {
            $this->attributes['delivery_date_time'] = null;
        }
    }
     ...
     * @return string
     */
    public function getDeliveryDateTimeAttribute($input)
    {
    ...
    {
        return $this->belongsToMany(Trailer::class, 'delivery_instruction_trailer')->withTrashed();
    }
    ...
    {
        return $this->belongsToMany(VehicleSc::class, 'delivery_instruction_vehicle_sc')->withTrashed();
    }
    ...
    public function load_descriptions() {
        return $this->hasMany(LoadDescription::class, 'delivery_instruction_number_id');
    }
}
