...
class MonthlyReportsController extends Controller
{
    public function index(Request $r)
    {
        ...
        foreach ($exp_group as $exp) {
            foreach ($exp as $line) {
                if (! isset($exp_summary[$line->expense_category])) {
                    $exp_summary[$line->expense_category] = [
                        'name'   => $line->expense_category,
                        'amount' => 0,
                    ];
                }
                $exp_summary[$line->expense_category]['amount'] += $line->amount;
            }
        }
        ...
        foreach ($inc_group as $inc) {
            foreach ($inc as $line) {
                if (! isset($inc_summary[$line->income_category])) {
                    $inc_summary[$line->income_category] = [
                        'name'   => $line->income_category,
                        'amount' => 0,
                    ];
                }
                $inc_summary[$line->income_category]['amount'] += $line->amount;
            }
        }
