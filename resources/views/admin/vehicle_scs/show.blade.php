        ...
        <div class="panel-body">
            <div class="panel panel-default">
                <div class="panel-body">
                    ...
                    <div class="row">
                        @if (config('invoices.logo_file') != '')
                            <div class="col-md-12 text-center">
                                ...
                                <h1><span style="color:#CE8F64">VENDOR VEHICLE</span></h1>
                                <h4><b>Vehicle reg No</b>: <span style="color:red">{{ $vehicle_sc->registration_number }}</span></h4>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="row">
                        <div class="col-xs-4 ">
                            <b>Vendor</b>: <span style="color:#CE8F64">{{ $vehicle_sc->vendor->name or '' }}</span><br>
                            @if ($vehicle_sc->subcontractor_number != '')
                                <b>Subcontractor No</b>: {{ $vehicle_sc->subcontractor_number->subcontractor_number or '' }}<br>
                            @endif
                        </div>
                        
                        <div class="col-xs-4 form-group text-center">
                            @if ($vehicle_sc->registration_number != '')
                                <b>Registration certificate</b>: @if($vehicle_sc->certificate_of_registration)<a href="{{ asset(env('UPLOAD_PATH').'/' . $vehicle_sc->certificate_of_registration) }}" target="_blank">Download file</a>@endif<br>
                            @endif
                            @if ($vehicle_sc->vehicle_type != '')
                                <b>Vehicle type</b>: {{ $vehicle_sc->vehicle_type }}<br>
                            @endif
                            @if ($vehicle_sc->make != '')
                                <b>Make</b>: {{ $vehicle_sc->make }}<br>
                            @endif
                            @if ($vehicle_sc->model != '')
                                <b>Model</b>: {{ $vehicle_sc->model }}<br>
                            @endif
                        </div>
                        <div class="col-xs-4 form-group text-right">
                             @if ($vehicle_sc->certificate_of_fitness_number != '')
                                <b>Certificate of fitness No</b>: {{ $vehicle_sc->certificate_of_fitness_number }}
                            @endif
                            @if($vehicle_sc->certificate_of_fitness)<a href="{{ asset(env('UPLOAD_PATH').'/' . $vehicle_sc->certificate_of_fitness) }}" target="_blank">Download file</a>@endif<br>
                            @if ($vehicle_sc->expiration_date != '')
                                <b>Expiry date</b>: {{ $vehicle_sc->expiration_date }}<br>
                            @endif
                            @if ($vehicle_sc->registration_number != '')
                                <b>Service history report</b>: @if($vehicle_sc->service_history_reports)<a href="{{ asset(env('UPLOAD_PATH').'/' . $vehicle_sc->service_history_reports) }}" target="_blank">Download file</a>@endif<br>
                            @endif
                            @if ($vehicle_sc->tracker_pin_details != '')
                                <b>Tracker login details</b>: {{ $vehicle_sc->tracker_pin_details }}<br>
                            @endif
                            <!--@if ($vehicle_sc->tracker_password != '')
                                <b>Tracker password</b>: {{ $vehicle_sc->tracker_password }}<br>
                            @endif-->
                        </div>
                    </div>
...
{!! Form::open(array(
        ...
        @endif
            ...
            <a href="{{ route('admin.vehicle_scs.index') }}" class="btn btn-default">@lang('global.app_back_to_list')</a>
        </div>
    </div>
@stop
