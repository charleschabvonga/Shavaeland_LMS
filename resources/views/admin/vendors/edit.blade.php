...
@section('content')
    ...
    {!! Form::model($vendor, ['method' => 'PUT', 'route' => ['admin.vendors.update', $vendor->id]]) !!}
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
                        ...
                        <div class="files-list">
                            @foreach($vendor->getMedia('directors_proof_of_residence') as $media)
                                <p class="form-group">
                                    ...
                                    <input type="hidden" name="directors_proof_of_residence_id[]" value="{{ $media->id }}">
                                </p>
                            @endforeach
                        </div>
                    </div>
                        ...
                        <div class="files-list">
                            @foreach($vendor->getMedia('tax_clearance') as $media)
                                <p class="form-group">
                                    <a href="{{ $media->getUrl() }}" target="_blank">{{ $media->name }} ({{ $media->size }} KB)</a>
                                    <a href="#" class="btn btn-xs btn-danger remove-file">Remove</a>
                                    <input type="hidden" name="tax_clearance_id[]" value="{{ $media->id }}">
                                </p>
                            @endforeach
                        </div>
                    </div>
                        ...
                        <div class="files-list">
                            @foreach($vendor->getMedia('company_registration') as $media)
                                <p class="form-group">
                                    <a href="{{ $media->getUrl() }}" target="_blank">{{ $media->name }} ({{ $media->size }} KB)</a>
                                    <a href="#" class="btn btn-xs btn-danger remove-file">Remove</a>
                                    <input type="hidden" name="company_registration_id[]" value="{{ $media->id }}">
                                </p>
                            @endforeach
                        </div>
                    </div>
                        ...
                        <div class="files-list">
                            @foreach($vendor->getMedia('company_proof_of_residents') as $media)
                                <p class="form-group">
                                    <a href="{{ $media->getUrl() }}" target="_blank">{{ $media->name }} ({{ $media->size }} KB)</a>
                                    <a href="#" class="btn btn-xs btn-danger remove-file">Remove</a>
                                    <input type="hidden" name="company_proof_of_residents_id[]" value="{{ $media->id }}">
                                </p>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="panel-body">
                    <table class="table table-bordered table-striped" id="tab_logic">
                        ...
                        <tbody id="vendor-contacts">
                            @forelse(old('vendor_contacts', []) as $index => $data)
                                @include('admin.vendors.vendor_contacts_row', [
                                    'index' => $index
                                ])
                            @empty
                                @foreach($vendor->vendor_contacts as $item)
                                    @include('admin.vendors.vendor_contacts_row', [
                                        'index' => 'id-' . $item->id,
                                        'field' => $item
                                    ])
            ...
            <div class="panel panel-default">
                <div class="panel-body">
                    <table class="table table-bordered table-striped" id="tab_logic">
                        <legend class="text-center"><span style="color:#CE8F64">TRANSPORTER REQUIREMENTS</span></legend>
                        <thead>
                        <tr>
                        ...
                        <tbody id="transporter-requirements">
                            @forelse(old('road_freight_sub_contractors', []) as $index => $data)
                                @include('admin.vendors.road_freight_sub_contractors_row', [
                                    'index' => $index
                                ])
                            @empty
                                @foreach($vendor->road_freight_sub_contractors as $item)
                                    @include('admin.vendors.road_freight_sub_contractors_row', [
                                        'index' => 'id-' . $item->id,
                                        'field' => $item
                                    ])
